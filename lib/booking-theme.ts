...
export function bookingBannerShellClass(themeId: BookingThemeId): string {
  switch (themeId) {
    case "reconnect":
      return "relative z-0 h-[240px] w-full shrink-0 overflow-hidden border-b border-emerald-900/35 bg-zinc-950";
    case "disconnect":
      return "relative z-0 h-[240px] w-full shrink-0 overflow-hidden border-b border-zinc-800 bg-zinc-950";
    default:
      return "relative z-0 h-[240px] w-full shrink-0 overflow-hidden border-b border-border bg-card";
  }
}
...
export function bookingBookedPillClass(themeId: BookingThemeId): string {
  ...
  switch (themeId) {
    case "disconnect":
      return cn(base, "border border-primary/30 bg-primary/10 text-primary");
    case "reconnect":
      return cn(
        base,
...
export function bookingOverlayBlockClass(
  ...
  if (themeId === "reconnect") {
    return cn(
      base,
      bookingFull ? "bg-zinc-950/80 backdrop-blur-sm" : "bg-transparent",
    );
  }
  if (themeId === "disconnect") {
    return cn(
      base,
      bookingFull ? "bg-black/80 backdrop-blur-sm" : "bg-transparent",
    );
  }
  return cn(
    base,
    bookingFull ? "bg-background/80 backdrop-blur-sm" : "bg-transparent",
  );
}
